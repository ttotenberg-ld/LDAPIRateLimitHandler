...
CALL_THRESHOLD = 5
'''
If the API call limit falls below this number, the calls will pause and retry once the limit is reset.
'''
...
def checkRateLimit(method, url, apikey, body):
    ...
    while tries > 0:
        ...
        try:
            response = requests.request(method, BASE_URL + url, headers = {'Authorization': apikey}, data = body)
